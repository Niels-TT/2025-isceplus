...
Why:
    ...
    This discovery query intentionally scans all flight directions and relative
    orbits for the AOI/time window (it does not apply stack-config direction/
    orbit constraints).
...
from stack_common import buffered_kml_to_wkt, parse_kml_points, read_aoi_buffer_m
...
def write_geometry_map(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "Missing dependency 'shapely'/'pyproj' required for map generation."
        ) from exc
    ...
    combined_mercator: dict[tuple[str, int, str], Any] = {
        key: transform(to_mercator.transform, geom) for key, geom in combined.items()
    }
    aoi_mercator = [to_mercator.transform(x, y) for x, y in aoi_points]
    buffered_aoi_mercator = None
    if buffered_aoi_wkt:
        try:
            buffered_geom = shapely_wkt.loads(buffered_aoi_wkt)
            if not buffered_geom.is_valid:
                buffered_geom = buffered_geom.buffer(0)
            if not buffered_geom.is_empty:
                buffered_aoi_mercator = transform(to_mercator.transform, buffered_geom)
    ...
    for idx, summary in enumerate(summaries):
        ...
        geom = combined_mercator.get(key)
        if geom is None:
            continue
        color = cmap(idx % 20)
        all_bounds.append(tuple(geom.bounds))
    ...
    if buffered_aoi_mercator is not None:
        handles.insert(
            0,
            Line2D(
                [0],
        ...
        legend_labels.insert(0, "AOI + Buffer")
    else:
        handles.insert(0, Line2D([0], [0], color="#0077b6", linewidth=2.2))
    ...
    if buffered_aoi_mercator is not None:
        bminx, bminy, bmaxx, bmaxy = buffered_aoi_mercator.bounds
        minx = min(minx, bminx)
        miny = min(miny, bminy)
        maxx = max(maxx, bmaxx)
        maxy = max(maxy, bmaxy)
    ...
    if ctx is None:
        raise RuntimeError(
            "Missing dependency 'contextily' required for basemap rendering. "
            "Install it in isce3-feb (mamba install -n isce3-feb -c conda-forge contextily)."
        )
    try:
        ...
        ax.set_ylim(miny - pad_y, maxy + pad_y)
        basemap_added = True
        basemap_provider = "CartoDB.Positron (contextily,no-cache)"
    ...
    if buffered_aoi_mercator is not None:
        for poly in iter_polygons(buffered_aoi_mercator):
            bx, by = poly.exterior.xy
            ax.fill(bx, by, color="#0077b6", alpha=0.06, zorder=6)
            ax.plot(
                bx,
                by,
                color="#005f73",
    ...
    ax.fill(aoi_x, aoi_y, color="#0077b6", alpha=0.14, zorder=8)
    ...
    return {
        ...
        "includes_aoi_buffer_outline": buffered_aoi_mercator is not None,
    }
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Discover Sentinel-1 geometry candidates for a project AOI/time window. "
            "Scans all directions/orbits/frames that intersect the AOI."
        )
    )
    parser.add_argument(
        "--repo-root",
    ...
    parser.add_argument(
        "--output-map",
        default="",
        help="Output map PNG path. Default: <outputs.root>/candidates/stack_candidates.png",
    )
    parser.add_argument(
        "--no-map",
        action="store_true",
    ...
    all_summaries = build_summaries(result_list)
    summaries = [s for s in all_summaries if s.unique_dates >= args.min_unique_dates]
    ...
    if args.output_map:
        ...
        output_map = output_root / "candidates" / "stack_candidates.png"
    else:
        output_map = None

    print(f"AOI KML: {kml_path}")
    print(f"AOI discovery buffer: {aoi_buffer_m:.1f} m")
    print("AOI map outline: project AOI (unbuffered).")
    print(f"Search window: {start} -> {end}")
    print("Discovery scope: all flight directions and relative orbits (no direction/orbit filter).")
    print(f"Total matching scenes: {len(result_list)}")
    print(f"Geometry groups (all): {len(all_summaries)}")
    print(f"Geometry groups (after min-unique-dates={args.min_unique_dates}): {len(summaries)}")
    hidden_count = len(all_summaries) - len(summaries)
    if hidden_count > 0:
        print(
            f"Note: {hidden_count} group(s) hidden by --min-unique-dates={args.min_unique_dates}. "
            "Use --min-unique-dates 1 to inspect all groups."
        )
    if summaries:
        print()
